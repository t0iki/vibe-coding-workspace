import type { Enemy, GameContent, GameStats, RuneActive, RuneSupport } from '@/types'

export class Game {
  content: GameContent
  stats: GameStats
  enemies: Enemy[]
  activeRunes: RuneActive[]
  ...
  calculateDPS() {
    ...
    this.supportRunes.forEach((support) => {
      if (support.effect?.more?.hit) {
        const moreMult = 1 + support.effect.more.hit
        baseDPS *= moreMult
      }
    })
